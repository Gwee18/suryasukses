...
{
    /**
     * Data produk (kategori & sub-produk) disalin persis dari struktur
     * konten resmi suryasukses.com, supaya halaman Product identik
     * dengan website resmi.
     *
     * Data ini sengaja disimpan di dalam controller (bukan tabel/migration
     * baru) supaya perubahan tetap terbatas pada file-file halaman Product.
     */
    private const CATEGORIES = [
        ...
        'housewares' => [
            ...
            'ranges' => [9, 10, 11, 12, 13, 14, 15, 16, 17],
        ],
        'roofing' => [
            'id' => 5,
            'title' => 'Roofing',
    ...
    private const RANGES = [
        ...
        7 => [
            ...
            'parent_id' => 3,
            'parent_slug' => 'specialty-nonwoven',
    ...
    {
        $first = self::CATEGORIES['packaging'];

        return Redirect::to('/home/product_landing/'.$first['id'].'/slug/packaging');
    }
    ...
    public function landing($id, string $slug)
    {
        ...
        $ranges = collect($category['ranges'])
            ->map(fn ($rangeId) => array_merge(['id' => $rangeId], self::RANGES[$rangeId]))
            ->values();
    ...
    public function range($id, $parentId, string $slug)
    {
        ...
        foreach (self::RANGES as $key => $data) {
            if ($data['slug'] === $slug && (int) $data['parent_id'] === (int) $parentId) {
                $range = $data;
                $rangeId = $key;
                break;
            }
        ...
        $category = self::CATEGORIES[$range['parent_slug']];

        return view('product.detail', [
            'range' => array_merge(['id' => $rangeId], $range),
            'category' => $category,
            'categorySlug' => $range['parent_slug'],
            'allCategories' => self::CATEGORIES,
            'allRanges' => self::RANGES,
        ]);
    }
